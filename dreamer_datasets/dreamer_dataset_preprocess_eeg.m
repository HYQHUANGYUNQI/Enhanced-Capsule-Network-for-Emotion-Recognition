...
for i = 1:23
    saldir = '../data/dreamer_data_eeg/stimuli(m,14)/';
    data_stimuli = data_loader.DREAMER.Data{1,i}.EEG.stimuli;  
    for j = 1:18
        data_stimuli{j,1} = data_loader.DREAMER.Data{1,i}.EEG.stimuli{j,1};   
        trial_name = ['data_stimuli',num2str(j)];
        eval([trial_name,'=data_stimuli{j,1}']);  
    end
    
    savepath = [saldir,'eeg', num2str(i),'_stimuli'];
...
for i = 1:23
    saldir = '../data/dreamer_data_eeg/baseline(m,14)/';
    data_baseline = data_loader.DREAMER.Data{1,i}.EEG.baseline; 
    for j = 1:18
        data_baseline{j,1} = data_loader.DREAMER.Data{1,i}.EEG.baseline{j,1};  
        trial_name = ['data_baseline',num2str(j)];
        eval([trial_name,'=data_baseline{j,1}']);   
    end
    
    savepath = [saldir,'eeg',num2str(i),'_baseline'];
...
for i = 1:23
    saldir = '../data/dreamer_data_eeg/label_arousal(18,1)/';
