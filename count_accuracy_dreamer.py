...
for label in ['valence']: # ,'arousal','dominance']:#['valence','arousal','dominance']:
    ...
    for sub in subjects:
        print(sub)
        xl = xlrd.open_workbook(r'./result_'+dataset_name + '_redo'+'/sub_dependent_'+model_version+'/'+debase+'/'+sub+'_'+label+epoch+'/summary_'+sub+'.xlsx')
        table = xl.sheets()[1]
...
writer = pd.ExcelWriter('./result_'+dataset_name+'_redo'+'/'+dataset_name+'_'+model_version+'_'+debase+'.xlsx')
